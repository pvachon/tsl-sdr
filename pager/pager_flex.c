 ...
 *      demod_flex.c
 *
 *      Copyright (C) 2017 Phil Vachon <phil@vachon.nyc>
...
static
unsigned int count_bits(unsigned int data)
{
    return __builtin_popcount(data);
}

static
int bch3121_fix_errors(struct Flex * flex, unsigned int * data_to_fix, char PhaseNo)
{
    TSL_BUG_ON(NULL == flex);
    ...
    int decode_error=bch_code_decode(flex->Decode.bch_code, recd);
    ...
    if (!decode_error) {
        ...
        int fixed = count_bits((*data_to_fix & 0x7FFFFFFF) ^ data);
        if (fixed>0) {
...
static
unsigned int flex_sync_check(struct Flex * flex, uint64_t buf)
{
    TSL_BUG_ON(NULL == flex);
    ...
    if (count_bits(marker ^ FLEX_SYNC_MARKER) < 4  && count_bits(codelow ^ codehigh) < 4 ) {
        retval=codehigh;
    } else {
        retval=0;
    }
...
static
unsigned int flex_sync(struct Flex * flex, unsigned char sym) {
    TSL_BUG_ON(NULL == flex);
    int retval=0;
...
static
void parse_alphanumeric(struct Flex * flex, unsigned int * phaseptr, char PhaseNo, int mw1, int mw2, int j)
{
    TSL_BUG_ON(NULL == flex);
...
static
void parse_numeric(struct Flex * flex, unsigned int * phaseptr, char PhaseNo, int mw1, int mw2, int j)
{
    TSL_BUG_ON(NULL == flex);
...
static
void parse_tone_only(struct Flex * flex, char PhaseNo)
{
    TSL_BUG_ON(NULL == flex);
...
static
void parse_unknown(struct Flex * flex, unsigned int * phaseptr, char PhaseNo, int mw1, int mw2)
{
    TSL_BUG_ON(NULL == flex);
...
static
void parse_capcode(struct Flex * flex, unsigned int aw1, unsigned int aw2)
{
    TSL_BUG_ON(NULL == flex);
...
static
void decode_phase(struct Flex * flex, char PhaseNo)
{
    TSL_BUG_ON(NULL == flex);
    ...
    for (i = aoffset; i < voffset; i++) {
        ...
        verbprintf(3, "FLEX: CAPCODE:%016x\n", flex->Decode.capcode);
...
static
void clear_phase_data(struct Flex * flex)
{
    TSL_BUG_ON(NULL == flex);
...
static
void decode_data(struct Flex * flex)
{
    TSL_BUG_ON(NULL == flex);
...
static
int read_data(struct Flex * flex, unsigned char sym)
{
    TSL_BUG_ON(NULL == flex);
...
static
void report_state(struct Flex * flex)
{
    TSL_BUG_ON(NULL == flex);
...
static
void flex_sym(struct Flex * flex, unsigned char sym)
{
    TSL_BUG_ON(NULL == flex);
...
void flex_demodulate(struct Flex * flex, double sample)
{
    ...
    if (flex->State.Current==FLEX_STATE_SYNC1) {
        flex->Modulation.zero=(flex->Modulation.zero*(flex->Demodulator.sample_freq*DC_OFFSET_FILTER) + sample) /
            ((flex->Demodulator.sample_freq*DC_OFFSET_FILTER)+1);
    }
...
void flex_delete(struct Flex * flex)
{
    ...
    if (flex->Decode.bch_code != NULL) {
        bch_code_delete(flex->Decode.bch_code);
        flex->Decode.bch_code=NULL;
    }
...
struct Flex * flex_new(unsigned int SampleFrequency)
{
    ...
    if (flex!=NULL) {
        ...
        TSL_BUG_IF_FAILED(bch_code_new(&flex->Decode.bch_code, p, 5, 31, 21, 2));
        if (flex->Decode.bch_code == NULL) {
            flex_delete(flex);
            flex=NULL;
        }
